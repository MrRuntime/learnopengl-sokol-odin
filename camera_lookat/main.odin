...
frame_cb :: proc "c" () {
	...
	radius: f32 = 10
	cam_x := glm.sin(f32(stm.sec(stm.now()))) * radius
	cam_z := glm.cos(f32(stm.sec(stm.now()))) * radius
	view := glm.mat4LookAt(glm.vec3{cam_x, 0, cam_z}, glm.vec3{0, 0, 0}, glm.vec3{0, 1, 0})
