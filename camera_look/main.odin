...
event_cb :: proc "c" (ev: ^sapp.Event) {
	...
	case .KEY_DOWN:
		on_keydown(ev.key_code)
	case .MOUSE_DOWN, .MOUSE_UP:
		state.mouse_btn = !state.mouse_btn
	case .MOUSE_MOVE:
		if state.mouse_btn do on_mouse_move(ev.mouse_x, ev.mouse_y)
	case .MOUSE_SCROLL:
		if state.fov >= 1 && state.fov <= 60 do state.fov -= ev.scroll_y

		if state.fov <= 1 {
			state.fov = 1
		} else if state.fov >= 60 {
			state.fov = 60
		}
	}
}

on_mouse_move :: proc(mouse_x, mouse_y: f32) {
	...
	xoffset := mouse_x - state.last_x
	yoffset := state.last_y - mouse_y
	state.last_x = mouse_x
	state.last_y = mouse_y
	...
	direction := glm.vec3 {
		...
		glm.sin(glm.radians(state.pitch)),
		glm.sin(glm.radians(state.yaw)) * glm.cos(glm.radians(state.pitch)),
	}
	state.cam_front = glm.normalize(direction)
}

on_keydown :: proc(key_code: sapp.Keycode) {
	...
	case .D:
		...
		state.cam_pos += offset
	case .SPACE:
		mouse_shown := sapp.mouse_shown()
		sapp.show_mouse(!mouse_shown)
	}
}
