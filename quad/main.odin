...
init_cb :: proc "c" () {
	...
	vertices := []Vec3{{0.5, 0.5, 0.0}, {0.5, -0.5, 0.0}, {-0.5, -0.5, 0.0}, {-0.5, 0.5, 0.0}}
	...
	state.pip = sg.make_pipeline(
		{shader = state.shader, index_type = .UINT16, layout = {attrs = {0 = {format = .FLOAT3}}}},
	)
...
frame_cb :: proc "c" () {
	...
	sg.apply_bindings(
		{vertex_buffers = {0 = state.vertex_buffer}, index_buffer = state.index_buffer},
	)
	sg.draw(0, 6, 1)
	sg.end_pass()
	sg.commit()
